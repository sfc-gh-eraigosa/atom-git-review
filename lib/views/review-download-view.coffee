...
class ReviewDownloadView extends View
  ...
  @content: ->
    @div class: 'overlay from-top', =>
      @div class: 'block', =>
        @subview 'gerritChange', new TextEditorView(mini: true, placeholderText: 'Gerrit change request')
      @div class: 'block', =>
        @subview 'gerritPatch', new TextEditorView(mini: true, placeholderText: '(optional) Patch Number')
      @div class: 'block', =>
  ...
  initialize: ->
    ...
    @disposables.add atom.commands.add 'atom-text-editor', 'core:confirm': (event) =>
      editor = @gerritChange.getModel()
      changeid = editor.getText()
      if changeid.length > 0
        downloadChange:(panel) ->
        destroy()
